...
class MinimalRot3:
    """Small substitute for gtsam.Rot3 (only what's needed here)."""
...
Rot3Like = "gtsam.Rot3" if _HAS_GTSAM else MinimalRot3  # for type checkers only
...
class PreintegrationResult:
    delta_t: float
    delta_R: object  # gtsam.Rot3 or MinimalRot3
    delta_v: np.ndarray  # (3,)
    delta_p: np.ndarray  # (3,)
    covariance: np.ndarray  # (9,9)
    delta_yaw: float  # convenience for planar usage
...
def build_default_params(
    ...
    """Return GTSAM params if available, else None."""
    if not _HAS_GTSAM:
        return None
    ...
    params.setBiasAccOmegaInt((1e-8) * np.eye(6))
    return params
...
def _so3_exp(w: np.ndarray) -> np.ndarray:
    """Rodrigues exponential map for small rotation vector w (rad)."""
...
class IMUPreintegrationWrapper:
    """Preintegrate IMU between two times.

    If GTSAM is installed -> uses `PreintegratedImuMeasurements`.
    Otherwise -> uses a simple SO(3) + kinematics integrator (no bias/cov).
    """
    ...
    def preintegrate(self, samples: Sequence[IMUSample], reset: bool = True) -> PreintegrationResult:
        if len(samples) < 2:
            return PreintegrationResult(
                delta_t=0.0,
                delta_R=gtsam.Rot3() if _HAS_GTSAM else MinimalRot3.identity(),
                delta_v=np.zeros(3),
                delta_p=np.zeros(3),
        ...
        if self._use_gtsam:
            for s0, s1 in zip(samples[:-1], samples[1:]):
                ...
                self._pim.integrateMeasurement(
                    np.asarray(s0.accel, dtype=float).reshape(3),
                    np.asarray(s0.omega, dtype=float).reshape(3),
                    dt,
                )
            pim = self._pim
            rot = pim.deltaRij()
            ...
            return PreintegrationResult(
                ...
                delta_R=rot,
                delta_v=np.array(pim.deltaVij(), dtype=float).reshape(3),
                delta_p=np.array(pim.deltaPij(), dtype=float).reshape(3),
                covariance=np.array(pim.preintMeasCov(), dtype=float),
                delta_yaw=dyaw,
            )

        # Fallback (no bias, no gravity removal). We keep delta_p in the initial body frame.
        for s0, s1 in zip(samples[:-1], samples[1:]):
            dt = float(s1.t - s0.t)
            if dt <= 0:
                continue
            ...
            # integrate accel in the *current* body frame mapped into the initial frame
            a0 = self._R @ accel
        ...
        _, _, dyaw = rot.rpy()
        return PreintegrationResult(
            delta_t=float(self._dt),
            delta_R=rot,
            delta_v=self._v.copy(),
            delta_p=self._p.copy(),
            covariance=np.zeros((9, 9)),
...
def rot3_yaw(rot: object) -> float:
    """Return yaw angle (rad) from either gtsam.Rot3 or MinimalRot3."""
    if _HAS_GTSAM and hasattr(rot, "rpy"):
        r, p, y = rot.rpy()
        return float(y)
    if isinstance(rot, MinimalRot3):
        return float(rot.rpy()[2])
    # last resort: assume matrix-like
